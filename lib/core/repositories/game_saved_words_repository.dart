  ...
  Future<bool> saveToWordsFromGames({
    ...
    try {
      ...
      final newPair =
          WordPair()
            ..id =
                0 // Will be assigned by savePairs
            ..setId = setId
      ...
      final allPairs = <WordPair>[...deduplicatedExisting.values, newPair];
  ...
  Future<int> removeDuplicatesFromWordsFromGames() async {
    try {
      ...
      for (final pair in existingPairs) {
        ...
        if (!normalizedPairs.containsKey(key) ||
            pair.id > normalizedPairs[key]!.id) {
...
final gameSavedWordsRepositoryProvider = Provider<GameSavedWordsRepository>((
  ref,
) {
