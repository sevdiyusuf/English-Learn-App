...
class _SharedWordSetPageState extends ConsumerState<SharedWordSetPage> {
  ...
  Widget _buildActualContent(dynamic repo) {
    return FutureBuilder<SharedWordSet?>(
      ...
      builder: (context, snapshot) {
        ...
        final isOwner =
            ref.watch(authControllerProvider).value?.uid == shared.ownerUid;
                        ...
                        : TextButton.icon(
                          onPressed:
                              () => showReportDialog(
                                ...
                                ref,
                                type: 'share',
                                targetId: shared.shareId,
                                title: l10n.reportSharedSet,
                              ),
  ...
  Future<void> _confirmRemove(
    ...
    final remove = await showDialog<bool>(
      ...
      builder:
          (_) => AlertDialog(
            title: Text(l10n.removeSharedSet),
            content: Text(l10n.removeSharedSetPrompt),
            actions: [
              TextButton(
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.sharedSetRemoved)));
