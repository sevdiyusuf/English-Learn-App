...
void main() {
  group('ArenaPlayer callable payload boundary tests', () {
    test('toCallablePayload produces primitive-only map with name and photoUrl', () {
    ...
    test('toCallablePayload strictly excludes client-authoritative id, score, and isOnline fields', () {
      final player = ArenaPlayer(
    ...
    test('toCallablePayload contains only callable-safe primitive or null values', () {
      final player = ArenaPlayer(
        id: 'user_primitive',
      ...
      for (final entry in payload.entries) {
        ...
        expect(
          ...
          isTrue,
          reason: 'Field ${entry.key} with value $val is not a primitive/null callable-safe type',
        );
      }
    });

    test('Preserves original ArenaPlayer.toJson Firestore TimestampConverter behavior', () {
      final now = DateTime.now();
      final player = ArenaPlayer(
        id: 'user_firestore',
